...
(function () {
  ...
  // ---- Internal migration (population view): net by region and district ----
  function divergingBars(items, o) {
    o = o || {};
    const f = d3.format(",");
    const W = o.W || 460, mL = o.mL || 118, mR = o.mR || 92, mT = 6;
    const rowH = o.rowH || 28, barH = o.barH || 15;
    const H = mT + items.length * rowH + 6;
    const halfW = (W - mL - mR) / 2, cx = mL + halfW;
    ...
    const svg = d3.create("svg").attr("viewBox", `0 0 ${W} ${H}`).attr("class", "imig-svg")
      .attr("role", "img").attr("aria-label", o.aria || "net migration");
    svg.append("line").attr("class", "imig-zero").attr("x1", cx).attr("x2", cx).attr("y1", mT).attr("y2", H - 6);
    items.forEach((r, i) => {
      const cy = mT + i * rowH + rowH / 2, gain = r.net >= 0, w = Math.max(1, x(Math.abs(r.net)));
      svg.append("text").attr("class", "imig-name").attr("text-anchor", "end").attr("x", mL - 10).attr("y", cy + 3.2)
        .style("font-size", (o.nameSize || 11.5) + "px").text(r.name);
      svg.append("rect").attr("class", gain ? "imig-gain" : "imig-loss")
        .attr("x", gain ? cx : cx - w).attr("y", cy - barH / 2).attr("width", w).attr("height", barH).attr("rx", 2.5);
      svg.append("text").attr("class", "imig-val").attr("text-anchor", gain ? "start" : "end")
        .attr("x", gain ? cx + w + 5 : cx - w - 5).attr("y", cy + 3.2)
        .style("font-size", (o.valSize || 10) + "px").text((gain ? "+" : "−") + f(Math.abs(r.net)));
    });
    return svg.node();
  }
  ...
  function renderInternalMigration() {
    ...
    const f = d3.format(",");
    const regions = im.regions.map(r => ({ name: r.name, net: r.in - r.out })).sort((a, b) => b.net - a.net);
    const districts = (im.districts || []).slice().sort((a, b) => b.net - a.net);
    const gainers = districts.filter(d => d.net > 0).length;

    el.innerHTML =
      `<h4 class="census-title">Inside the country, everyone moves to Chișinău</h4>`
      + `<div class="census-sub">At the census, ${f(im.total)} people had moved to a different district. Net internal migration, the arrivals minus the departures, leaves the capital as the only big winner among the development regions.</div>`
      + `<div id="imigRegions"></div>`
      + `<h4 class="census-title census-title-2">A closer look, district by district</h4>`
      + `<div class="census-sub">Only ${gainers} districts gained people. They are Chișinău's suburban ring (Ialoveni, Anenii Noi, Criuleni, Strășeni) and the northern city of Bălți. Every other district lost people, mostly to the capital.</div>`
      + `<div id="imigDistricts"></div>`
    ...
    const rHost = el.querySelector("#imigRegions");
    if (rHost) rHost.appendChild(divergingBars(regions, { rowH: 30, barH: 16, mL: 118, mR: 96, aria: "Net internal migration by development region" }));
    const dHost = el.querySelector("#imigDistricts");
    if (dHost) dHost.appendChild(divergingBars(districts, { rowH: 15, barH: 9, mL: 96, mR: 76, nameSize: 9.5, valSize: 8.5, aria: "Net internal migration by district" }));
    el.hidden = false;
  }
